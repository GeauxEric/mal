...
def _is_macro_call(ast, env: Env):
    match ast:
        case LispList([s, *_]):
            if isinstance(s, LispSymbol) and env.find(s):
                fn = env.get(s)
...
def _macroexpand(ast, env):
    while True:
        if _is_macro_call(ast, env):
            match ast:
                case LispList([s, *rest]):
                    fn: LispClosure = env.get(s)
                    ast = fn.value(*rest)
                    continue
                case _:
                    raise RuntimeError("Failed to get macro function")
        else:
            return ast
        ...
        match ast:
            case LispList(v):
                ...
                match v[0]:
                    ...
                    case LispSymbol('macroexpand'):
                        return _macroexpand(v[1], repl_env)
                    case LispSymbol('defmacro!'):
                        value = EVAL(v[2], repl_env=repl_env)
                        value.is_macro = True
                        return repl_env.set(v[1], value)
                    case LispSymbol('let*'):
